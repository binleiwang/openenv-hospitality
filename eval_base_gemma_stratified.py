"""
Cross-family null test: eval BASE Gemma-2-9B-IT on the stratified set.

Qwen 7B base, v1, v2, v3 all clustered at +0.101 (per-task IDENTICAL across
all 20 tasks between v1 and base). Qwen 14B base came in at +0.068 — no
capacity benefit, just a different failure mode (tool-spam instead of freeze).

Remaining ambiguity: is +0.101 a Qwen-family quirk, or is it the env-data
ceiling for any ~8B open-weights base at this trajectory count?

  If Gemma 9B base >> +0.15: Qwen-family-specific failure. Try Gemma SFT.
  If Gemma 9B base ~= +0.10: null result is cross-family. Env data is the
      bottleneck. Ship the honest story.

15-25 min on H100, ~$1. Third leg of the triple-family null test
(Qwen-7B, Qwen-14B, now Gemma-9B; Llama-8B runs alongside).

Usage:
  cd /home/ubuntu/openenv-hospitality
  python eval_base_gemma_stratified.py
"""
...
BASE_MODEL = "unsloth/gemma-2-9b-it-bnb-4bit"   # <-- cross-family null test (Google)
MAX_SEQ_LENGTH = 8192
...
EVAL_OUT = "/home/ubuntu/hospitality/eval/eval_heldout_base_gemma_stratified.json"
...
print(f"  Gemma 2 9B BASE (no SFT):     {mean_reward:+.3f}  ← THIS RUN")
print()
_delta_vs_qwen7b = mean_reward - 0.101
print(f"Gemma 9B vs Qwen 7B base: Δ = {_delta_vs_qwen7b:+.3f}")
if mean_reward >= 0.15:
    print("  → Qwen-family-specific failure. Gemma actually learns. Try Gemma SFT.")
elif abs(_delta_vs_qwen7b) < 0.05:
    print("  → TRIPLE-FAMILY NULL CONFIRMED (Qwen-7B ≈ Qwen-14B ≈ Gemma-9B).")
