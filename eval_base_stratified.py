"""
Control experiment: eval the BASE model (no SFT, no LoRA) on the v3
stratified eval set, using the same SYSTEM_PROMPT_V2 + rollout_v3 inference
stack as all SFT checkpoints.

Why this is the most important experiment we haven't run:
  v1 stratified: +0.101
  v2 stratified: +0.133
  v3 stratified: +0.104
  Claude:        +0.314

  All three SFT checkpoints cluster within 0.03 of each other (noise on a
  20-task eval). This means either:
    (a) SFT is working but hits a ceiling at ~+0.10 (base is lower)
    (b) SFT is doing ~nothing and base is already at +0.10
    (c) SFT is actively harming some things, helping others, net zero

  We need base number to disambiguate. 15 min, ~$0.7 on Lambda.

Usage:
  cd /home/ubuntu/openenv-hospitality
  python eval_base_stratified.py
"""
...
EVAL_OUT = "/home/ubuntu/hospitality/eval/eval_heldout_base_stratified.json"
...
print(f"  v1 (3ep, 37 traj):        +0.101")
print(f"  v2 (3ep, 50 traj):        +0.133")
print(f"  v3 (6ep, 40 traj, strat): +0.104")
print(f"  BASE (no SFT):            {mean_reward:+.3f}  ← THIS RUN")
