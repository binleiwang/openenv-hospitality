...
from .base import DB, BaseModelNoExtra
...
class HospitalityDB(DB):
    ...
    # Policy lookup tracking (which get_*_policy tools were called)
    policy_lookups_made: List[Dict[str, Any]] = Field(
        default_factory=list, description="Record of policy tool lookups performed"
    )
