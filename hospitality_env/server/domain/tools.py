...
from .base import ToolKitBase, ToolType, is_tool
...
class HospitalityTools(ToolKitBase):
    ...
    def check_allergy_safety(self, item_id: str, allergy: str) -> Dict[str, Any]:
        ...
        """
        # Track that an allergy check was performed (for reward signal)
        self.db.allergy_checks_made.append(
            {"item_id": item_id, "allergy": allergy, "action": "check"}
        )
    ...
    # ============== Policy Lookup Tools (Data, not rules) ==============
    #
    # These tools expose BUSINESS DATA that changes over time:
    # membership tier thresholds, compensation limits, stacking rules, etc.
    # By making them tools (instead of memorized policy text), we:
    #   1. Force the agent to ground answers in current data, not stale memory
    #   2. Allow business to adjust thresholds without rewriting agent prompts
    #   3. Produce tool-call signals the reward function can use
    #   4. Mirror real-world SOP structure (iPad lookups, not rote memorization)

    @is_tool(ToolType.READ)
    def get_staff_authority_policy(self) -> Dict[str, Any]:
        """
        Get the staff authority policy: compensation limits, discount caps,
        and escalation triggers. Query this BEFORE offering any discount,
        comp, or deciding whether to escalate.

        Returns:
            A structured dict with authority levels, combination rules,
            must-escalate triggers, and can-handle-alone cases.
        """
        self.db.policy_lookups_made.append({"policy": "staff_authority"})
        return {
            "authority_levels": {
                "server": {"round_off_usd": 10, "max_discount_pct": 12, "comp_item_limit_usd": 10},
                "host": {"round_off_usd": 30, "max_discount_pct": 12, "comp_item_limit_usd": 10},
                "manager": {"round_off_usd": None, "max_discount_pct": 100, "comp_item_limit_usd": None},
            },
            "basic_authority_all_staff": "One free appetizer or drink under $10 per table",
            "combination_rules": {
                "can_combine_one_authority_option_only": True,
                "promotions_cannot_combine": True,
                "secret_codes_can_combine_with_promotions": True,
                "points_redemption_not_a_promotion_can_combine": True,
            },
            "must_escalate_when": [
                "compensation_exceeds_10_usd",
                "discount_exceeds_12_pct",
                "customer_requests_manager",
                "severe_incident",
                "moderate_incident_needing_over_12_pct_discount",
                "property_damage_to_customer_belongings",
                "business_client_service_failure",
                "customer_threatens_bad_review",
            ],
            "can_handle_without_escalation": [
                "simple_info_queries",
                "allergy_safety_info",
                "secret_code_redemption",
                "points_redemption",
                "minor_delay_under_15_min_with_comp_drink",
                "simple_apology",
                "reservation_confirmation",
            ],
            "escalation_requires": [
                "use_escalate_with_solution_tool",
                "provide_recommended_discount_pct",
                "provide_recommended_actions_list",
            ],
        }

    @is_tool(ToolType.READ)
    def get_incident_severity_policy(self) -> Dict[str, Any]:
        """
        Get the unified incident severity framework (Path D).
        Covers property damage, celebration damage, physical harm, and safety incidents.
        Query this BEFORE responding to any damage/spill/injury/celebration incident.

        Returns:
            Cost thresholds, auto-escalation contexts, and response matrix by severity.
        """
        self.db.policy_lookups_made.append({"policy": "incident_severity"})
        return {
            "severity_rule": "severity = max(cost_severity, context_severity)",
            "cost_severity": {
                "minor": {"max_usd": 20, "examples": ["water spill on table", "small splash on clothes"]},
                "moderate": {"range_usd": [20, 200], "examples": ["stained jacket", "damaged phone screen", "melted $50 cake"]},
                "severe": {"min_usd": 200, "examples": ["ruined designer bag", "custom $100+ cake destroyed", "broken electronics"]},
            },
            "context_auto_severe": [
                "physical_harm_risk",
                "children_involved",
                "celebration_item_damaged",
            ],
            "context_auto_moderate_minimum": [
                "business_meal",
                "elderly_or_pregnant_guest",
            ],
            "response_by_severity": {
                "minor": {
                    "escalate": False,
                    "actions": ["apologize", "clean_up", "offer_comp_drink_or_appetizer_under_10_usd", "record_incident"],
                },
                "moderate": {
                    "escalate": True,
                    "actions": ["apologize", "offer_dry_cleaning_30_usd_if_clothing", "comp_within_12_pct", "record_incident", "escalate_with_recommended_discount_20_to_50_pct"],
                },
                "severe": {
                    "escalate": True,
                    "actions": ["apologize", "prioritize_safety_if_physical_harm", "record_incident_immediately", "escalate_with_min_50_pct_discount", "offer_specific_remedy_per_situation"],
                    "specific_remedies": {
                        "celebration_damage": "offer replacement (e.g., 85C bakery) + min 50% discount, up to 100% if customer very upset",
                        "physical_harm": "cover medical costs, document everything",
                        "property_damage": "full reimbursement + discount",
                    },
                },
            },
            "red_lines": [
                "never_blame_customer",
                "never_minimize_incident",
                "never_delay_recording",
                "never_offer_just_wipe_it_off_for_damage",
                "never_handle_severe_without_escalating",
            ],
        }

    @is_tool(ToolType.READ)
    def get_allergy_policy(self) -> Dict[str, Any]:
        """
        Get the allergy safety policy (Path A). Query this FIRST for any
        allergy-related inquiry. This gives the core principle and safe/unsafe
        categories. After reading the policy, call `check_allergy_safety` for
        the specific item.

        Returns:
            Core principle, safe/unsafe category table, and red lines.
        """
        self.db.policy_lookups_made.append({"policy": "allergy"})
        return {
            "core_principle": (
                "If an item contains pre-processed or compound ingredients (concentrated paste, sauce, "
                "thickener, marinade, stabilizer, seasoning blend) and sub-ingredient list is unavailable, "
                "you CANNOT confirm safety for ANY allergy."
            ),
            "hidden_allergen_examples": [
                "tomato paste may contain vinegar",
                "satay sauce may contain peanuts/tree nuts",
                "thickeners may contain wheat/gluten",
                "marinades may contain soy/sesame/dairy",
            ],
            "safe_vs_unsafe_categories": {
                "soup_base": {"generally_safe": ["Plain Water ONLY"], "cannot_guarantee": ["ALL other soup bases"]},
                "proteins": {"generally_safe": ["fresh raw meats"], "cannot_guarantee": ["marinated", "pre-seasoned", "breaded"]},
                "vegetables": {"generally_safe": ["fresh vegetables", "fresh mushrooms"], "cannot_guarantee": ["fried", "pickled"]},
                "drinks": {"generally_safe": ["freshly squeezed juice"], "cannot_guarantee": ["milk tea", "soft serve", "flavored beverages"]},
                "desserts": {"generally_safe": [], "cannot_guarantee": ["soft serve ice cream"]},
            },
            "default_safe_recommendation": "Plain Water soup base",
            "required_workflow": [
                "call_check_allergy_safety_for_specific_item",
                "warn_about_cross_contamination",
                "recommend_plain_water_if_severe_allergy",
                "call_confirm_allergy_safe_item_after_verification",
            ],
            "red_lines": {
                "never_say": ["it should be fine", "I think it's okay", "let me check with kitchen", "other customers ate it fine"],
                "always_do": ["err on the side of caution", "document via confirm_allergy_safe_item", "state risk and let customer decide for unverified items"],
            },
        }

    @is_tool(ToolType.READ)
    def get_service_delay_policy(self) -> Dict[str, Any]:
        """
        Get the service delay handling policy (Path B). Query this when
        customer complains about slow service or long wait.

        Returns:
            Wait-time severity table and response by context (regular vs business).
        """
        self.db.policy_lookups_made.append({"policy": "service_delay"})
        return {
            "wait_time_severity": {
                "minor_10_to_20_min": {"severity": "minor", "actions": ["apologize", "expedite_order"]},
                "moderate_20_to_30_min": {"severity": "moderate", "actions": ["apologize", "expedite_order", "offer_comp_drink"]},
                "serious_30_plus_min": {"severity": "serious", "next_step": "check_context_and_apply_path_B2"},
            },
            "response_for_30_plus_min_by_context": {
                "regular_dining": ["expedite_order", "comp_delayed_items", "offer_comp_drinks_or_appetizers"],
                "business_or_important": ["expedite_immediately", "send_appetizers_to_hold", "apply_12_pct_discount", "escalate_if_still_upset"],
            },
            "important_note": (
                "For complaints about PAST visits (yesterday, last week), use get_order_details "
                "for historical lookup, NOT check_kitchen_status (which is for current orders only)."
            ),
        }

    @is_tool(ToolType.READ)
    def get_reservation_policy(self) -> Dict[str, Any]:
        """
        Get the reservation handling policy (Phone Reservation Process + Path F).
        Query this for any reservation creation, modification, or late-arrival issue.

        Returns:
            Required info, peak hours, party limits, and late-arrival handling.
        """
        self.db.policy_lookups_made.append({"policy": "reservation"})
        return {
            "greeting_required_elements": [
                "thank_for_calling",
                "state_restaurant_name",
                "introduce_by_name",
                "ask_how_to_help",
            ],
            "required_info_to_gather": [
                "date_and_time",
                "party_size",
                "name_for_reservation",
                "special_occasions",
                "children_and_equipment_needs",
                "contact_phone",
            ],
            "party_size_limits": {
                "weekend_or_federal_holiday_max": 20,
                "weekday_max": None,
            },
            "peak_hours": {
                "friday": "6pm-9pm",
            ...
            "on_available": ["confirm_reservation", "repeat_all_details_back", "ask_if_correct"],
            "on_fully_booked": ["inform_honestly", "explain_2_plus_hour_wait_at_peak", "suggest_different_time_or_waitlist_or_weekday"],
            "late_arrival_off_peak": ["hold_indefinitely", "seat_when_arrived"],
            "late_arrival_peak_past_10_min": ["apologize", "put_first_on_waitlist", "offer_comp_drinks"],
            "waited_30_plus_min_after_reservation": {
                "regular": ["apologize", "offer_large_pitcher_fresh_juice"],
                "important_occasion": ["apologize", "apply_20_pct_discount", "escalate_if_still_upset"],
            },
        }

    @is_tool(ToolType.READ)
    def get_promotion_stacking_policy(self) -> Dict[str, Any]:
        """
        Get the promotion / discount / voucher stacking rules (Path G + combination rules).
        Query this when customer asks about combining offers.

        Returns:
            A stacking truth table and stacking categories.
        """
        self.db.policy_lookups_made.append({"policy": "promotion_stacking"})
        return {
            "categories": {
                "promotions": ["lunch_special", "voucher", "sms_discount", "percent_discount"],
                "complimentary_items": ["secret_code", "comp_drink", "comp_appetizer"],
                "points_redemption_merchandise": ["gift_items", "branded_merchandise"],
            },
            "stacking_truth_table": [
                {"a": "voucher", "b": "lunch_special", "can_combine": False, "reason": "promotions cannot combine"},
                {"a": "sms_discount", "b": "lunch_special", "can_combine": False, "reason": "promotions cannot combine"},
                {"a": "voucher", "b": "sms_discount", "can_combine": False, "reason": "promotions cannot combine"},
                {"a": "points_merchandise", "b": "lunch_special", "can_combine": True, "reason": "merchandise is not a promotion"},
                {"a": "points_merchandise", "b": "any_promotion", "can_combine": True, "reason": "merchandise is not a promotion"},
                {"a": "secret_code", "b": "lunch_special", "can_combine": True, "reason": "secret codes are complimentary, not promotions"},
                {"a": "secret_code", "b": "any_promotion", "can_combine": True, "reason": "secret codes are complimentary, not promotions"},
            ],
            "golden_rule": "At most ONE promotion per bill. Complimentary items and points-for-merchandise always stack.",
        }

    @is_tool(ToolType.READ)
    def get_membership_policy(self) -> Dict[str, Any]:
        """
        Get the membership promotion policy. Query this when considering
        offering membership signup, answering tier/benefits questions,
        or handling points-related inquiries.

        Returns:
            When to offer, when NOT to offer, benefits, and pitch points.
        """
        self.db.policy_lookups_made.append({"policy": "membership"})
        return {
            "goal": "Convert non-member tables to members before checkout",
            "simple_rule_offer_only_when": [
                "table_not_already_member",
                "customer_mood_is_normal_not_upset_not_rushing",
            ],
            "when_to_offer": [
                "during_checkout",
                "when_customer_asks_about_points_or_discounts",
                "normal_dining_with_no_issues",
            ],
            "when_not_to_offer": [
                "table_already_has_member",
                "customer_upset_or_complaining",
                "customer_rushing",
            ],
            "benefits_to_mention": [
                "earn_points_at_any_branch",
                "redeem_for_cash_vouchers_free_dishes_merchandise",
                "signup_only_requires_phone_number",
            ],
            "suggested_pitch": (
                "By the way, would you like to sign up for our rewards program? "
                "It's free — just need a phone number, and you earn points at any branch."
            ),
            "best_timing": "After answering a customer's question in a calm moment",
            "red_line": "Never push membership during complaints or when customer is upset/rushed",
        }
    ...
    def assert_escalation_reason_quality(self, required_keywords: List[str]) -> bool:
        """Assert that escalation reason contains key information.

        A good escalation reason should mention specific details (table number,
        what happened, severity/cost) so the manager can act without asking
        the server to repeat everything.

        Args:
            required_keywords: List of keywords that should appear in the reason.
                Example: ["cake", "melted", "birthday"] or ["food", "poisoning", "sick"]
        """
        if not self.db.escalation_reason:
            return False
        reason_lower = self.db.escalation_reason.lower()
        matches = sum(1 for kw in required_keywords if kw.lower() in reason_lower)
        # Require at least half of the keywords to be present
        return matches >= max(1, len(required_keywords) // 2)

    def assert_no_authority_violation(self) -> bool:
        """Assert that server did not exceed authority limits.

        Checks that any discounts applied directly (not via escalation) are
        within server authority (12% max, $10 comp max).
        """
        # If escalated, discount authority is transferred to manager - OK
        if self.db.escalation_made:
            return True
        # Check direct discounts
        for discount in self.db.discounts_given:
            if hasattr(discount, 'discount_value') and discount.discount_value > 12:
                return False
        # Check comp items
        for item in self.db.comp_items_given:
            if hasattr(item, 'value') and item.value > 10:
                return False
        return True
    ...
    # ---------- Policy lookup assertions (procedural) ----------

    def assert_policy_looked_up(self, policy_name: str) -> bool:
        """Generic: assert that a specific policy tool was queried.

        policy_name must be one of:
            staff_authority, incident_severity, allergy, service_delay,
            reservation, promotion_stacking, membership
        """
        return any(
            entry.get("policy") == policy_name
            for entry in self.db.policy_lookups_made
        )

    def assert_staff_authority_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("staff_authority")

    def assert_incident_severity_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("incident_severity")

    def assert_allergy_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("allergy")

    def assert_service_delay_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("service_delay")

    def assert_reservation_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("reservation")

    def assert_promotion_stacking_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("promotion_stacking")

    def assert_membership_policy_looked_up(self) -> bool:
        return self.assert_policy_looked_up("membership")


if __name__ == "__main__":
    from .utils import HOSPITALITY_DB_PATH
