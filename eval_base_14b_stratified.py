"""
Stage 1 of 14B experiment: eval BASE Qwen2.5-14B-Instruct on stratified set.

On 7B we established that v1 = v2 = v3 = base = +0.101 (per-task IDENTICAL).
SFT did zero work on 7B. The remaining question: is the Claude gap
(+0.31 vs +0.10) a CAPACITY bottleneck or an ENV-DATA bottleneck?

  If 14B base >> +0.10: capacity. Bigger model cracks the reasoning-heavy
      categories (food_safety, incident, host_phone) on its own.
  If 14B base ~= +0.10: env data itself is insufficient. Bigger model
      doesn't help because the bottleneck is upstream of the model.

15-25 min on H100, ~$1. Decides whether to run Stage 2 (14B + SFT).

Usage:
  cd /home/ubuntu/openenv-hospitality
  python eval_base_14b_stratified.py
"""
...
BASE_MODEL = "unsloth/Qwen2.5-14B-Instruct-bnb-4bit"   # <-- THE CHANGE vs 7B script
MAX_SEQ_LENGTH = 8192
...
EVAL_OUT = "/home/ubuntu/hospitality/eval/eval_heldout_base_14b_stratified.json"
...
print(f"  7B v1 (SFT, 37 traj):         +0.101")
print(f"  7B v2 (SFT, 50 traj):         +0.133")
print(f"  7B v3 (SFT, 40 traj, strat):  +0.104")
print(f"  7B BASE (no SFT):             +0.101")
print(f"  14B BASE (no SFT):            {mean_reward:+.3f}  ← THIS RUN")
print()
_delta_vs_7b = mean_reward - 0.101
print(f"14B vs 7B base: Δ = {_delta_vs_7b:+.3f}")
if _delta_vs_7b >= 0.15:
    print("  → CAPACITY WAS THE BOTTLENECK. Stage 2 (14B + SFT) is worth running.")
elif _delta_vs_7b >= 0.05:
    print("  → Capacity helps a little. Stage 2 may push further.")
else:
    print("  → Capacity is NOT the bottleneck. Env data is the real limit.")
    print("    Do NOT run Stage 2 — write up the null result instead.")
