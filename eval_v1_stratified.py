"""
Standalone eval of v1 (binleiwang/qwen2.5-7b-hospitality-sft) on the v3
stratified eval set.

Why this script exists:
  v1 was evaluated on tasks[100:115] (ID-biased) and reported +0.338.
  v2 and v3 were evaluated on stratified_eval_ids.json (stratified holdout).
  We can't honestly compare v1 +0.338 against v3 +0.104 because they used
  different eval sets. This script runs v1 on the SAME stratified eval set
  so all three checkpoints are directly comparable.

Inference setup matches train_lambda.py §7 exactly:
  - SYSTEM_PROMPT_V2 (anti-loop + "is there anything else" rule)
  - rollout_v3 (per-turn trace capture, nudge after 2 tool-less turns)
  - greedy decoding, max 8 turns
  - Same env server on 127.0.0.1:8000

Usage on Lambda (in Terminal, inside a kernel-restart-clean shell):
  cd /home/ubuntu/openenv-hospitality
  python eval_v1_stratified.py

Prereq: env already installed (done by Section 2 of train_lambda.py in a
prior run). Server does NOT need to be running — this script launches it.
"""

# ---------------------------------------------------------------------------
# CONFIG — paths mirror train_lambda.py
# ---------------------------------------------------------------------------
import os
...
V1_HUB_REPO = "binleiwang/qwen2.5-7b-hospitality-sft"   # the v1 adapter
BASE_MODEL = "unsloth/Qwen2.5-7B-Instruct-bnb-4bit"      # base that v1 was LoRA-fine-tuned on
MAX_SEQ_LENGTH = 8192
...
EVAL_OUT = "/home/ubuntu/hospitality/eval/eval_heldout_v1_stratified.json"
...
# ---------------------------------------------------------------------------
# 1. Start env server (subprocess, uses this Python's site-packages)
# ---------------------------------------------------------------------------
import sys, subprocess, time, httpx

print("Starting env server on 127.0.0.1:8000 ...")
...
for _ in range(30):
    ...
    raise RuntimeError(f"Server failed to come up.\nstdout:\n{out_tail}\nstderr:\n{err_tail}")


# ---------------------------------------------------------------------------
# 2. Load v1 adapter on top of base model
# ---------------------------------------------------------------------------
...
print(f"\nLoading base model {BASE_MODEL} + v1 adapter {V1_HUB_REPO} ...")
# Unsloth way to load a LoRA from Hub: pass the adapter repo as model_name.
# Unsloth detects adapter_config.json and auto-loads the right base.
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name=V1_HUB_REPO,
    max_seq_length=MAX_SEQ_LENGTH,
    load_in_4bit=True,
    dtype=torch.bfloat16,
)
FastLanguageModel.for_inference(model)
print("Model loaded, inference mode.")


# ---------------------------------------------------------------------------
# 3. Agent wrapper — SAME as train_lambda.py §6 (verbatim)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Load stratified eval set + Claude baseline
# ---------------------------------------------------------------------------
...
# 5. Run eval
# ---------------------------------------------------------------------------
print(f"\nRunning v1 on {len(EVAL_IDS)} stratified eval tasks ...")
results = []
for tid in EVAL_IDS:
...
print(f"Claude on SAME stratified eval: +0.314  (real target)")
print(f"v1 on OLD ID-biased eval:       +0.338  (inflated, not comparable)")
print(f"v2 on stratified eval:          +0.133 (undertrained)")
print(f"v3 on stratified eval:          +0.104 (overfit, loss 0.06)")
...
# Kill server (clean exit so next run doesn't hit port-in-use)
try:
